...
| Real Instrument song creation | The automation14 MCP chain created a new Empty project and a 120-bar arrangement with 6 Instrument Tracks, 6 MIDI Parts, 3,924 notes, 2 Group Tracks, 2 FX Tracks, a Marker Track, exact HALion programs, and validated routing | development evidence complete; non-release profile |
| Real song audibility safety | Cubase Pro 14.0.32 passed every per-track solo meter probe and the final MixConsole probe; `cubase.export_run` produced a 3:28.70 WAV that passed duration/format/hash/RMS/LUFS/true-peak/silence analysis | development evidence complete; exact safe14 evidence pending |
